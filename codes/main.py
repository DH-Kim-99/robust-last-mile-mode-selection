...
def run_sensitivity_analysis(instance_type='toy', gamma_values=None):
    ...
    for gamma in gamma_values:
        try:
            ...
            temp_file = os.path.join(config.results_dir, f"sensitivity_{instance_type}_temp.csv")
    ...
    output_file = os.path.join(config.results_dir, f"sensitivity_{instance_type}_{timestamp}.csv")
...
def plot_sensitivity_results(df_results, instance_type='toy'):
    ...
    try:
        ...
        # Save plot
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        plot_file = os.path.join(config.results_dir, f"sensitivity_{instance_type}_{timestamp}.png")
...
def main():
    ...
    if seed is not None:
        data_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data', f"data_{instance_type}_seed{seed}.pkl")
        if not os.path.exists(data_file):
            print(f"Error: Data file not found: {data_file}")
            print(f"Available seeds for {instance_type}: ", end="")
            import glob
            available = glob.glob(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data', f"data_{instance_type}_seed*.pkl"))
            if available:
